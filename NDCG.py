...
def run_multiple_Ps(p_limit):
    ...
    for p in range(1, p_limit + 1):
        ...
        report_data[p]["Vector Space"]["Average NDCG"] = reduce(lambda a, b: a + b, vector_space_score) / len(
            vector_space_score)
...
if __name__ == "__main__":
    ...
    p_values = p_score(list(NDCG.values()))
    average_NDCG = reduce(lambda a, b: a + b, p_values) / len(p_values)


    print(f"Average NDCG for P{p}: {'{:.3f}'.format(average_NDCG)}")
